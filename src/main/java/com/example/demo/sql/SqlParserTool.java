...
public class SqlParserTool {
    ...
    public static List<Table> getIntoTables(SelectBody selectBody) {
        if (selectBody instanceof PlainSelect) {
            ...
            List<Table> tables = select.getIntoTables();
            return tables;
        }
    ...
    public static void main(String[] args) throws JSQLParserException {
        String sql = "select " +
                ...
                " from t_user t1" +
                " left join t_user_info t2 on t1.id=t2.user_id" +
        ...
        if (sqlType.equals(SqlType.SELECT)) {
            ...
            System.out.println(new Gson().toJson(tables));
            System.out.println();

            List<SelectItem> selectItems = SqlParserTool.getSelectItems(statement.getSelectBody());
            FromItem fromItem = SqlParserTool.getFromItem(statement.getSelectBody());
            Table table = (Table) fromItem;
            Alias tableAlias = table.getAlias();
            String tableName = table.getName();

            List<Table> intoTables = SqlParserTool.getIntoTables(statement.getSelectBody());
            boolean multiSubSelect = SqlParserTool.isMultiSubSelect(statement.getSelectBody());

            List<Join> joins = SqlParserTool.getJoins(statement.getSelectBody());

            for (Join join : joins) {
                Table table1 = (Table) join.getRightItem();
                String name1 = table1.getName();
                Alias alias1 = table1.getAlias();
                System.out.println();
            }
